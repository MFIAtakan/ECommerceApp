    ...
    public class ShoppingCardController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse>> GetShoppingCart(string userId)
        {
            try
            {
                ...
                {
                    shoppingCart = _db.ShoppingCards
        ...
        public async Task<ActionResult<ApiResponse>> AddOrUpdateItemInCart(string userId, int menuItemId, int updateQuantityBy)
        {
            ...
            ShoppingCard shoppingCart = _db.ShoppingCards.Include(u => u.CartItems).FirstOrDefault(u => u.UserId == userId);
            ...
            if (shoppingCart == null && updateQuantityBy > 0)
            {
                ...
                ShoppingCard newCart = new() { UserId = userId };
                _db.ShoppingCards.Add(newCart);
                _db.SaveChanges();

                CardItem newCartItem = new()
                {
            ...
            else
            {
                ...
                CardItem cartItemInCart = shoppingCart.CartItems.FirstOrDefault(u => u.MenuItemId == menuItemId);
                ...
                else
                {
                    ...
                    if (updateQuantityBy == 0 || newQuantity <= 0)
                    {
                        ...
                        _db.CardItems.Remove(cartItemInCart);
                        if (shoppingCart.CartItems.Count() == 1)
                        {
                            _db.ShoppingCards.Remove(shoppingCart);
                        }
                        _db.SaveChanges();
                    }
                    else
